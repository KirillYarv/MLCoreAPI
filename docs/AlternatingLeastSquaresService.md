...
|---|---|---|---|---|---|
| `AlternatingLeastSquaresService` | `__init__` | `als_repo: AlsRepository`, `cache_prefix: str = 'data_for_als'`, `factors: int = 20`, `regularization: float = 0.01`, `iterations: int = 20`, `pool_processes: int = 3`, `use_gpu: bool = True` | public | `None` | Инициализирует параметры ALS, префикс файловых артефактов и размеры матрицы на основе БД. |
| `AlternatingLeastSquaresService` | `get_recommendations` | `top_k: int` | public | `list[dict[str, Any]]` | Возвращает рекомендации из файловых артефактов модели или запускает полный pipeline расчета. |
| `AlternatingLeastSquaresService` | `refresh_recommendations` | `top_k: int = 12` | public | `int` | Удаляет файловые артефакты модели и пересчитывает рекомендации заново; возвращает количество записей. |
...
| `AlternatingLeastSquaresService` | `_get_repository_data` | `transactions_postfix: str` | private | `pd.DataFrame` | Загружает данные одной партиции из БД и преобразует их в DataFrame (`t_dat`, `customer_id`, `article_id`). |
| `AlternatingLeastSquaresService` | `_calculate_recommendations` | `top_k: int` | private | `list[dict[str, Any]]` | Проверяет GPU, параллельно загружает партиции, собирает единый DataFrame, фильтрует данные и обучает ALS. |
