...
| `get_pairs` | `Нет` | public | `dict[str, Any]` | Endpoint `/api/pairs`, возвращает только уникальные пары; в `message` пишет их количество (`unique_pairs`). ARL использует общий `JsonFileCacheService` для чтения/записи кеша. |
...
| `get_als_recommendations` | `user_id: str`, `top_k: int = 12` | public | `dict[str, Any]` | Endpoint `/api/als/recommendations/{user_id}`, загружает общий ALS-кеш, фильтрует по пользователю, возвращает `user_not_found`/`gpu_unavailable` при ошибках. ALS использует общий `JsonFileCacheService` для кешей результатов. |
